...
class TestMovieEndPoints(APITestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="mag2", password="qwerty1234@#", email="mag2@mag.com"
        )
        ...
        self.genre2 = Genre.objects.create(name="war")
    ...
    def test_filter_movies_by_genre(self):
        """Test the filter movie views by genre"""
        url = reverse("filter-movies-by-genre")
        response = self.client.get(url, args=[self.genre2])

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn(self.movie1.name, response.data["results"][0]["name"])
        self.assertEqual(self.genre2.name, response.data["results"][0]["genre"]["name"])

    def test_filter_series_by_year(self):
        """Test the filter movie view by"""
        url = reverse("filter-series-by-year") + "?y=1234"
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(self.movie2.year, response.data["results"][0].get("year"))
